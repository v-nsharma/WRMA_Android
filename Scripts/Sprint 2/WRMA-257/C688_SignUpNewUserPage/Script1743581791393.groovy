...
TestData testData = findTestData('Data Files/C688 (1)')
...
String lastName = testData.getValue('lastNamee', 1)
...
'Begins a try block to handle potential exceptions.'
try {
    Mobile.tap(findTestObject('Object Repository/Repo/android.widget.Button - Next (2)'), 0)

    println('Button is not clickable as expected.')

    assert true : 'Button is not clickable as expected.'
}
catch (Exception e) {
    println("Button is clickable, but it should not be. Exception: $e.getMessage()")
} 
